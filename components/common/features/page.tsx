...
const allFeatures = [
  ...
  },
  {
    ...
    description:
      "Built with healthcare-grade security from the ground up: 256-bit AES encryption, HIPAA compliance, role-based access control, and automated daily cloud backups.",
  ...
  return (
    <div className="bg-background text-foreground">
        ...
        <section className="relative py-28 px-4 sm:px-6 lg:px-8 overflow-hidden">
          ...
          <div
            className="absolute bottom-10 right-10 w-80 h-80 bg-gradient-to-br from-purple-500/20 to-primary/30 rounded-full blur-3xl animate-morph-blob opacity-20"
            style={{ animationDelay: "4s" }}
          />
          <div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-gradient-to-br from-emerald-500/10 to-cyan-500/10 rounded-full blur-3xl animate-morph-blob opacity-15"
          ...
          <div className="max-w-5xl mx-auto text-center relative z-10 space-y-8">
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-primary/10 border border-primary/20 text-primary text-sm font-semibold animate-reveal-up"></div>

            <h1
              className="text-5xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight leading-tight animate-reveal-up stagger-1"
            ...
            <div
              className="flex flex-wrap justify-center gap-4 pt-4 animate-reveal-up stagger-3"
              style={{ animationFillMode: "both" }}
            >
              <Link href="/#pricing">
                <Button
                  size="lg"
                  className="bg-primary hover:bg-primary/90 text-lg font-semibold shadow-xl hover:shadow-2xl transition-all hover:scale-105"
                >
              ...
              <Link href="/#gallery">
                <Button
                  size="lg"
                  variant="outline"
                  className="text-lg font-semibold hover:bg-primary/5"
                >
                  View Live Demo
                </Button>
              </Link>
          ...
          <div className="max-w-4xl mx-auto text-center relative z-10 space-y-8">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Link href="/#pricing">
                <Button
                  size="lg"
                  className="bg-primary hover:bg-primary/90 text-lg font-semibold shadow-xl"
                >
...
function FeatureSection({
  ...
  const VectorComponent = feature.vector;
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute ${
          isEven ? "top-10 -right-20" : "top-10 -left-20"
        } w-96 h-96 bg-gradient-to-br ${
          feature.color
        } rounded-full blur-3xl opacity-10 animate-morph-blob`}
      />
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div
                className={`p-3 rounded-xl ${feature.bgColor} border border-current/10`}
              >
                <feature.icon className={`w-6 h-6 ${feature.iconColor}`} />
              </div>
              <span
                className={`text-xs font-bold uppercase tracking-widest ${feature.iconColor}`}
              >
            ...
            <p
              className={`text-xl font-medium bg-gradient-to-r ${feature.color} bg-clip-text text-transparent`}
            >
              {feature.tagline}
            </p>
            ...
            <ul className="space-y-3">
              {feature.highlights.map((item, i) => (
                <li key={i} className="flex items-start gap-3 group">
                  <CheckCircle2
                    className={`w-5 h-5 ${feature.iconColor} flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform`}
                  />
            ...
            <div className="grid grid-cols-3 gap-4 pt-4">
              {feature.stats.map((stat, i) => (
                <div
                  key={i}
                  className="text-center p-4 rounded-xl bg-card border border-border hover:border-primary/50 transition-colors group"
                >
                  <p
                    className={`text-2xl font-extrabold bg-gradient-to-r ${feature.color} bg-clip-text text-transparent group-hover:scale-105 transition-transform`}
                  >
                    {stat.value}
                  </p>
            ...
            <Link
              ...
              className={`inline-flex items-center gap-2 text-sm font-semibold ${feature.iconColor} hover:underline transition-all group`}
            >
          ...
          >
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
              <div className="relative w-[400px] h-[400px]">
                {[0, 1, 2, 3].map((i) => (
                  ...
                  >
                    <Star
                      className={`w-3 h-3 ${feature.iconColor} opacity-40`}
                    />
                  </div>
                ))}
              </div>
            ...
            {/* Main screenshot */}
            <div className="relative rounded-2xl overflow-hidden border-2 border-border shadow-2xl group hover:shadow-3xl transition-shadow duration-500">
              <div
              ...
              <div
                className={`absolute bottom-4 left-4 px-4 py-2 rounded-xl bg-gradient-to-r ${feature.color} text-white text-xs font-bold shadow-lg flex items-center gap-2`}
              >
