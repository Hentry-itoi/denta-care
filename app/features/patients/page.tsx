...
import { Users, ArrowRight, ArrowLeft, Heart, FileText, Shield, UserCheck, Baby } from 'lucide-react'
      ...
      <main className="pt-16">
          ...
          <div className="max-w-7xl mx-auto relative z-10 grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6 animate-reveal-left" style={{ animationFillMode: 'both' }}>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-tight">
                ...
                <span className="bg-gradient-to-r from-primary via-emerald-400 to-cyan-500 bg-clip-text text-transparent">Patient Records</span>
              </h1>
              <p className="text-xl text-foreground/75 max-w-lg leading-relaxed font-medium">
                One unified profile for every patient—medical history, dental charts, X-rays, 
                treatment plans, prescriptions, insurance, and family connections.
              </p>
              <div className="flex flex-wrap gap-4 pt-2">
                  <Button linkHref='/contact' size="lg" className="h-14 px-8 sm:px-10 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground text-base sm:text-lg font-bold shadow-xl shadow-primary/25 hover:shadow-2xl transition-all hover:scale-105">
                    Start Managing Patients <ArrowRight className="ml-2 w-5 h-5" />
                  </Button>
              </div>
            </div>
            <div className="animate-reveal-right" style={{ animationFillMode: 'both', animationDelay: '0.2s' }}>
              <div className="relative rounded-2xl overflow-hidden border-2 border-primary/30 shadow-2xl bg-card">
                <Image src="/features/users.png" alt="Patient Management" width={1200} height={800} className="w-full h-auto" priority />
        ...
        <section ref={sec1.ref} className="py-20 px-4 sm:px-6 lg:px-8 bg-muted/20 border-b border-border/80">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className={`text-center space-y-4 ${sec1.isVisible ? 'animate-reveal-up' : 'opacity-0'}`} style={{ animationFillMode: 'both' }}>
              <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight">Everything About Your Patients, In One Place</h2>
            </div>
            <div className={`grid sm:grid-cols-2 lg:grid-cols-3 gap-6 ${sec1.isVisible ? 'animate-reveal-up' : 'opacity-0'}`} style={{ animationFillMode: 'both', animationDelay: '0.2s' }}>
              {[
                { icon: Heart, title: 'Medical History', desc: 'Track allergies, conditions, medications, and past treatments for informed care decisions.' },
                { icon: FileText, title: 'Treatment Plans', desc: 'Build multi-visit treatment plans with cost estimates, progress tracking, and patient consent.' },
                { icon: Shield, title: 'Insurance Management', desc: 'Verify coverage, manage claims, and track reimbursements for every patient.' },
                { icon: UserCheck, title: 'Digital Dental Charts', desc: 'Interactive tooth-by-tooth charting with condition, procedure, and notes for each tooth.' },
                { icon: Baby, title: 'Family Accounts', desc: 'Link family members together for pediatric and family dentistry workflows.' },
                { icon: Users, title: 'Communication Logs', desc: 'Track all patient communications—calls, messages, emails—in one unified timeline.' }
              ].map((cap, i) => (
                <div key={i} className="group bg-card border border-border/80 rounded-2xl p-6 space-y-4 hover:border-primary/50 hover:shadow-xl transition-all duration-300">
                  ...
                  <p className="text-sm text-foreground/75 font-medium leading-relaxed">{cap.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 px-4 sm:px-6 lg:px-8 text-center space-y-8">
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight">
            Ready to <span className="bg-gradient-to-r from-primary to-emerald-400 bg-clip-text text-transparent">Know Every Patient?</span>
          </h2>
