...
import { CreditCard, ArrowRight, ArrowLeft, FileText, DollarSign, Receipt, Send, Percent, Wallet } from 'lucide-react'
      ...
      <main className="pt-16">
          ...
          <div className="max-w-7xl mx-auto relative z-10 grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6 animate-reveal-left" style={{ animationFillMode: 'both' }}>
              ...
              <div className="flex w-fit items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-bold uppercase tracking-wider">
                <CreditCard className="w-4 h-4" /> Feature 03 — Billing
              </div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-tight">
                <span className="text-foreground">Automated </span>
                <span className="bg-gradient-to-r from-primary via-emerald-400 to-cyan-500 bg-clip-text text-transparent">Billing & Invoices</span>
              </h1>
              <p className="text-xl text-foreground/75 max-w-lg leading-relaxed font-medium">
                Professional dental invoices with itemized treatment breakdowns. Process payments via 
                cash, card, UPI, or insurance—and track overdue balances automatically.
              </p>
                <Button size="lg" linkHref='/contact' className="h-14 px-8 sm:px-10 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground text-base sm:text-lg font-bold shadow-xl shadow-primary/25 hover:shadow-2xl transition-all hover:scale-105">
                  Start Billing Smarter <ArrowRight className="ml-2 w-5 h-5" />
                </Button>
            </div>
            <div className="animate-reveal-right" style={{ animationFillMode: 'both', animationDelay: '0.2s' }}>
              <div className="relative rounded-2xl overflow-hidden border-2 border-primary/30 shadow-2xl bg-card">
                <Image src="/features/invoices.png" alt="Billing & Invoices" width={1200} height={800} className="w-full h-auto" priority />
        ...
        <section ref={sec1.ref} className="py-20 px-4 sm:px-6 lg:px-8 bg-muted/20 border-b border-border/80">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className={`text-center space-y-4 ${sec1.isVisible ? 'animate-reveal-up' : 'opacity-0'}`} style={{ animationFillMode: 'both' }}>
              <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight">Billing That Works While You Treat</h2>
            </div>
            <div className={`grid sm:grid-cols-2 lg:grid-cols-3 gap-6 ${sec1.isVisible ? 'animate-reveal-up' : 'opacity-0'}`} style={{ animationFillMode: 'both', animationDelay: '0.2s' }}>
              {[
                { icon: FileText, title: 'Itemized Invoices', desc: 'Auto-generate detailed invoices with procedure codes, descriptions, and pricing.' },
                { icon: Wallet, title: 'Multi-Payment Methods', desc: 'Accept cash, cards, UPI, net banking, and insurance payouts seamlessly.' },
                { icon: Send, title: 'Auto Payment Reminders', desc: 'Overdue invoices trigger automated SMS and email reminders to patients.' },
                { icon: Receipt, title: 'PDF & Print', desc: 'Download branded PDF invoices or print receipts directly from the dashboard.' },
                { icon: Percent, title: 'Tax & Discounts', desc: 'Built-in GST/VAT calculation and flexible discount rules per procedure.' },
                { icon: DollarSign, title: 'Payment Links', desc: 'Send one-click payment links via WhatsApp or email for instant patient checkout.' }
              ].map((cap, i) => (
        ...
        <section ref={sec2.ref} className="py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-primary/10 via-emerald-500/10 to-cyan-500/10 border-b border-border/80">
          <div className={`max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center ${sec2.isVisible ? 'animate-reveal-up' : 'opacity-0'}`} style={{ animationFillMode: 'both' }}>
            {[
              ...
              { value: '5s', label: 'Invoice generation' }
            ].map((stat, i) => (
        ...
        <section className="py-20 px-4 sm:px-6 lg:px-8 text-center space-y-8">
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight">
            Ready to <span className="bg-gradient-to-r from-primary to-emerald-400 bg-clip-text text-transparent">Get Paid Faster?</span>
          </h2>
